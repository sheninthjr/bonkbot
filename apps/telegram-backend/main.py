...
async def handle_button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('copy_eth_'):
        ...
        response = requests.get(f'{BACKEND_URL}/user/privatekey/ETH/?telegramId={telegram_id}')
        parsed_response = json.loads(response.content)
        await context.bot.send_message(
            chat_id=query.message.chat_id,
        ...
        telegram_id = query.from_user.id
        response = requests.get(f'{BACKEND_URL}/user/privatekey/SOL/?telegramId={telegram_id}')
        parsed_response = json.loads(response.content)
        await context.bot.send_message(
            chat_id=query.message.chat_id,
        ...
        telegram_id = query.from_user.id
        response = requests.get(f'{BACKEND_URL}/user/seedphrase?telegramId={telegram_id}')
        parsed_response = json.loads(response.content)
        await context.bot.send_message(
            chat_id=query.message.chat_id,
        ...
        response = requests.get(f'{BACKEND_URL}/user/balance/ETH/?telegram_id={telegram_id}')
        parsed_response = json.loads(response.content)
        ...
        telegram_id = query.from_user.id
        await query.message.reply_text("Fetching...")
        response = requests.get(f'{BACKEND_URL}/user/balance/SOL/?telegram_id={telegram_id}')
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data['waiting_for'] == 'sol_address':
        ...
        try:
            ...
            parsed_response = json.loads(response.content)
            if response.status_code == 200:
                await update.message.reply_text(f"Transfer successful! Transaction hash: {parsed_response['hash']}")
            else:
                await update.message.reply_text(f"Transfer failed: {parsed_response['message']}")
